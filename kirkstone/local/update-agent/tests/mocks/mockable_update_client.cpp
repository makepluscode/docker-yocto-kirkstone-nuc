/**
 * @file mockable_update_client.cpp
 * @brief 모킹 가능한 UpdateClient 래퍼 구현
 */

#include "mockable_update_client.h"

MockableUpdateClient::MockableUpdateClient(const std::string& server_url,
    ...
    // 실제 UpdateClient 생성 (파싱 기능은 실제 구현 사용)
    real_agent_ = std::make_unique<UpdateClient>(server_url, tenant, device_id);
}

bool MockableUpdateClient::pollForUpdates(std::string& response) {
...
bool MockableUpdateClient::downloadBundle(const std::string& download_url, const std::string& local_path) {
    if (mock_http_client_) {
        // 모킹된 HTTP 클라이언트 사용
...
bool MockableUpdateClient::sendFeedback(const std::string& execution_id, const std::string& status, const std::string& message) {
    if (mock_http_client_) {
        // 모킹된 HTTP 클라이언트 사용
...
bool MockableUpdateClient::sendProgressFeedback(const std::string& execution_id, int progress, const std::string& message) {
    if (mock_http_client_) {
        // 모킹된 HTTP 클라이언트 사용
        std::string feedback_url = server_url_ + "/" + tenant_ + "/controller/v1/" + device_id_ + "/deploymentBase/" + execution_id + "/feedback";
...
bool MockableUpdateClient::sendStartedFeedback(const std::string& execution_id) {
    return sendFeedback(execution_id, "started", "Update started");
}

bool MockableUpdateClient::sendFinishedFeedback(const std::string& execution_id, bool success, const std::string& message) {
    std::string status = success ? "finished" : "error";
    return sendFeedback(execution_id, status, message);
}

bool MockableUpdateClient::parseUpdateResponse(const std::string& response, UpdateInfo& update_info) {
    // 파싱 기능은 실제 UpdateClient 구현 사용
